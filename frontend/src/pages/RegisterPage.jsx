...
const RegisterPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            showSnackbar('Identity created. Proceed to authentication.', 'success');
            navigate('/login');
        } catch (err) {
            const errMsg = err.response?.data?.message || 'Registration failed: Matrix initialization error';
            setError(errMsg);
            ...
            <div className="w-full max-w-md animate-in fade-in slide-in-from-bottom-6 duration-700">
                <div className="premium-card !p-10 md:!p-14 shadow-2xl">
                    <div className="text-center mb-12">
                        ...
                        <p className="text-slate-500 font-medium">Initialize your financial ecosystem profile.</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            <label className="block text-[11px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Alias / Username</label>
                        ...
                        <div>
                            <label className="block text-[11px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Communication ID (Email)</label>
                            <div className="relative group">
                        ...
                        <div>
                            <label className="block text-[11px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Secret Passphrase</label>
                            <div className="relative group">
                        ...
                        >
                            {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : 'Register Identity'}
                        </button>
                    </form>

                    <div className="mt-12 pt-8 border-t border-slate-50 text-center">
                        <p className="text-slate-500 text-sm font-semibold">
                            Already registered? <Link to="/login" className="text-indigo-600 font-black hover:text-indigo-700 underline underline-offset-8">Sign In</Link>
                        </p>
                    </div>
                </div>
            </div>
